...
const conexaoBD = mysql.createPool({
  host: 'localhost',
  user: 'root',
  password: '123456',
...
app.post('/login', async (req, res) => {
  usuario = await Cadastro.findOne({
    where: { email_cli_cli: req.body.email_cli_cli, senha_cli: req.body.senha_cli }
  });
  if (usuario === null) {
    res.send(JSON.stringify('Usuário não encontrado'));
  } else {
